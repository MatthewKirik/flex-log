...
const buildMessage = (loggingLevel, messageParts, entities) => {
  ...
  sb.append(messageParts[messageParts.length - 1]).append('\n');
...
const getLatestFile = async filePath => {
  ...
  const fileNames = await fh
    .readdir(dirPath)
    .then(names => names.filter(name => name.startsWith(baseFileName)));

  if (fileNames.length === 0) return null;

  const fileTimePromises = fileNames.map(async fileName => ({
    fileName,
    mtime: (await fh.stat(fileName)).mtime,
  }));

  return Promise.all(fileTimePromises).then(fileTimes =>
    fileTimes.reduce((latest, curr) =>
      (latest.mtime > curr.mtime ? latest : curr)
    )
  );
};
...
const appendMessage = async (filePath, latestFileName, message) => {
  const dirPath = fh.dirname(filePath);
  const appendPath = dirPath + '/' + latestFileName;
...
const processMessage = async (filePath, rollInterval, message) => {
  ...
  if (rollIntervalPassed) {
    await appendMessage(filePath, latestFile.fileName, message);
  } else {
    await writeMessage(filePath, message);
  }
};
