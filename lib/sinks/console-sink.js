...
const writeMessage = async (loggingLevel, levelColors, message, writer) => {
  let color = levelColors.get(loggingLevel);
  if (!color) color = Colors.FgColors.Black;
  const capsLogLevel = loggingLevel.toUpperCase();
  const colorLogLevel = `${color}${capsLogLevel}${Colors.Style.Reset}`;
...
const createSink = options => {
  ...
  const sink = async (loggingLevel, messageParts, entities) => {
    ...
    await writeMessage(loggingLevel, options.levelColors,
      message, options.writer);
  };
  return sink;
};
